...
export function MermaidDiagram({ code }: { code: string }) {
  ...
  const [svg, setSvg] = useState<string>(() => cache.get(key) ?? "");
  ...
  useEffect(() => {
    ...
    getMermaid(dark)
      .then((mermaid) =>
        mermaid.render(`mmd-${Math.random().toString(36).slice(2)}`, code.trim()),
      )
      .then(({ svg }) => {
        if (!alive) return;
        cache.set(key, svg);
  ...
  return (
    <div className="mermaid-diagram-wrap group">
      <div
        ref={ref}
      ...
      <div className="block-action-wrap opacity-0 group-hover:opacity-100">
        <button onClick={copyCode} className={`block-action-btn${copied ? " check" : ""}`} title="Copy code">
